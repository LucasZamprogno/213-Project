  ...
  @Override protected void execute () throws InvalidInstructionException, MachineHaltException, RegisterSet.InvalidRegisterNumberException, MainMemory.InvalidAddressException
  {
    switch (insOpCode.get()) {
      ...
      case 0x4: // st rs, (rd, ri, 4) .... 4sdi
    	int addrx4 = reg.get(insOp1.get()) + reg.get(insOp2.get()) * 4;
        mem.writeInteger(addrx4, reg.get(insOp0.get()));
        break;
      case 0x6: // ALU ................... 6-sd
	switch (insOp0.get()) {
	...
	break;
      case 0x7: // sh? $i,rd ............. 7dii
        byte shiftByte = (byte) ((byte) insOp1.get() << 4 | (byte) insOp2.get());
        int shiftVal = ((int) shiftByte) & 0xFF;
        if (shiftByte > 0) {
        	reg.set(reg.get(insOp2.get()), reg.get(insOp2.get()) << shiftVal);
        } else if (shiftByte < 0) {
        	reg.set(reg.get(insOp2.get()), reg.get(insOp2.get()) >> shiftVal);
        }
        break;
